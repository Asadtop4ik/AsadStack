...
def test_parameterized_route(app, test_client):
    ...
    assert test_client.get("http://testserver/hello/Asad").text == "Hello Asad"
    assert test_client.get("http://testserver/hello/PDP").text == "Hello PDP"
...
def test_class_based_get(app, test_client):
    ...
    assert test_client.get("http://testserver/books").text == "That is books page"
...
def test_class_based_post(app, test_client):
    ...
    assert test_client.post("http://testserver/books").text == "That is books post page"
...
def test_class_based_method_not_allowed(app, test_client):
    ...
    class Books:
        def post(self, req, resp):
            resp.text = "That is books post page"
...
def test_alternative_route_adding(app, test_client):
    ...
    assert test_client.get("http://testserver/new").text == "That is new handler"
...
def test_non_existent_static_file(test_client):
    assert test_client.get("http://testserver/static/nonexistent").status_code == 404
...
def test_json_response_helper(app, test_client):
    ...
    def json_handler(req, resp):
        resp.json = {"name": "Asadbek"}

    resp = test_client.get("http://testserver/json")
    resp_data = resp.json()

    assert resp.headers["Content-Type"] == "application/json"
    assert resp_data["name"] == "Asadbek"
